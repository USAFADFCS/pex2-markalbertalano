/** CPUs.c
 * ===========================================================
 * Name: <Last Name, First Name> alano, mark
 * Section: <Section> m3
 ...
 * ===========================================================
 * Documentation Statement: <describe any help received> cited on site
 * =========================================================== */
...
void* SJFcpu(void* param) {
    ...
    while (1) {
        ...
        if (p == NULL) {
            ...
            // the longest (qInsert always appends to the tail, so the head is
            // always the oldest arrival — that is the FIFO selection rule).
            p = qRemove(&(svars->readyQ), qShortest(&(svars->readyQ))); //this needs to change
...
void* SRTFcpu(void* param) {
    ...
    while (1) {
        ...
        if(p != NULL){
            //need to figure out where this if statement goes (inside vs outside/after if ==)
            if(p->priority > qShortestBR(&(svars->readyQ))){
